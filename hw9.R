...
optionlist <- list(title="Car Models Comparison bubble chart", xvar="displacement", yvar="miles per gallon")
bub <- gvisBubbleChart(mtcars, "model", "disp", "mpg", "gear", "hp", optionlist)
...
Motion <- gvisMotionChart(worldDat, idvar="country", timevar="year", xvar="life.expectancy", yvar="fertility.rate", colorvar="region", sizevar="population", 
                          options=list(title="Life Expectancy and Fertility Rate in Different Countries", xvar="life exopectancy", yvar="fertitlity rate"))
