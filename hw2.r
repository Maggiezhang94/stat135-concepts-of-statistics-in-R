...
plot(wr1500m$new_year, wr1500m$times_sec, type = "s", xlim = c(1892,2020))
...
text(1944, wr_1944, levels(droplevels(wr1500m[wr1500m$year == 1944, ]$athlete)), pos = 2, cex = 1, col = "grey")
text(1998, wr_1998, levels(droplevels(wr1500m[wr1500m$year == 1998, ]$athlete)), pos = 2, cex = 1, col = "grey")
...
plot(wr1500m$new_year, wr1500m$times_sec, xlim = c(1892, 2020), main = "World Record in Men's 1500 meter run", xlab = "Year", ylab = "Times (sec)", type = "s")
lines(c(1998, 2014), c(wr_1998, wr_1998))
abline(v = 1944, col = "grey")
abline(v = 1998, col = "grey")
text(1944, wr_1944, levels(droplevels(wr1500m[wr1500m$year == 1944, ]$athlete)), pos = 2, cex = 1, col = "grey")
...
### 1. The values are clustered at bottom left, so the data doesn't stand out
### 2. The plot is of poor scale, the values on x-axis and y-axis are hard to read
### 3. There are lots of empty space on the plot
...
symbols(log(SO2012Ctry$pop), log(GDP_per_person), circles = sqrt(SO2012Ctry$Total))
...
symbols(log(Medals$pop), log(Medals$GDP_per_person), circles = sqrt(Medals$Total)) 
points(log(No_Medals$pop), log(No_Medals$GDP_per_person), pch=10, cex=0.5)
...
top5 <- order(Medals$Total, decreasing = TRUE)[1:5]

symbols(log(Medals$pop), log(Medals$GDP_per_person), circles = sqrt(Medals$Total), xlab = "population (log)", ylab = "GDP per person (log)", main = "relationship between number of medals, GDP and population of countries in 2012 Olympics")
points(log(No_Medals$pop), log(No_Medals$GDP_per_person), pch=10, cex=0.5)
text(log(Medals[top5, ]$pop), log(Medals[top5, ]$GDP_per_person), labels = Medals[top5, ]$Country, pos = 2, cex = 0.8)
######################################
# PLOT 3.
...
wonMedal <- SO2012Ctry[SO2012Ctry != 0, ]
world <- map(col = "light grey", fill = TRUE)
names(wonMedal)
...
world <- map(col = "light grey", fill = TRUE)
symbols(wonMedal$longitude, wonMedal$latitude, circles = sqrt(wonMedal$Total)*4, add = TRUE, bg = myGold)
...
class(athletes$Name)
class(athletes$Sex)
class(athletes$Sport)
class(athletes$Country)
...
### beside should be set to TRUE to provide the easiest comparison.
...
barplot(athTab[ , orderSport], beside = TRUE, las = 2, cex.names = 0.8, main = "Relationship between sex and sports on London 2012 Olympics")
